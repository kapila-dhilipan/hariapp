...
from .utils import get_request_count
...
class TrackerAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'details', 'timestamp', 'ip_address_link',
    ...
    def details(self, obj):
        ...
        return format_html('<a href="{0}">Details</a>'.format(
            reverse('admin:authentication_tracker_change', args=(obj.pk,))))
        ...
        if obj.ip_address:
            return format_html(
                '<a href="{0}?ip_address__exact={1}">{1}</a>'.format(
                    reverse('admin:authentication_tracker_changelist'),
                    obj.ip_address,
                )
            )
    ...
    def user_link(self, obj):
        ...
        if obj.user:
            return format_html(
                '<a href="{0}?user__username__exact={1}">{1}</a>'.format(
                    reverse('admin:authentication_tracker_changelist'),
                    obj.user
                )
            )
    ...
    #     extra_context = extra_context or {}
    #     countries_count = []
    #     response = super(TrackerAdmin, self).changelist_view(request, extra_context)
    ...
    #         current_results = get_request_count(
